...
## Shared infrastructure
  ...
  - `forge.py` — batched GraphQL PR/MR fetch for GitHub + GitLab behind the interchangeable `Gh`/`Gl` backends; `start_fetch` + `resolve_pr_state`, the daemon-thread fetch seam both CLIs drive their live listings from; forge detection via `forge_kind`/`gitlab_fullpath`; `rollup_checks`; `IGNORED_WHEN_PENDING` + `set_ignored_when_pending` (empty by default, filled from config at startup — never hardcode a check name, it's specific to one repo's CI); GitHub-only native-stack methods on `Gh` (see below).
  - `ui.py` — colors + `set_color`, PR glyphs + `set_glyphs`, `pr_chip`, `redraw_prefix` (the cursor-rewind escape both live listings redraw with), `glyph_key` (the help-page symbol legend, shared so both tools document the same vocabulary), `pick`.
...
- Run with `python3 -m unittest discover tests` — stdlib only, no deps, 3.9+.
  ...
  - `wt`'s machine-readable output (`list --json`, `current --json`, consumed by the Sublime plugin) forks at the same pure-function seam as the table: `build_list_json` is the twin of `render_list`, both fed by `build_rows`. Test the payload by parsing it, never by string-matching. **Both `wt list` and `stack list` print-then-rewrite**: each paints its local view immediately with dim loading chips, never waiting on the forge, and redraws the PR column in place once the fetch lands, so the fetch gets a budget (`_LIST_PR_TOTAL`) far past what a blocking wait could afford. `render_list`/`render_stacks` stay pure; `print_list_live`/`print_stacks_live` own the cursor work, gated on `sys.stdout.isatty()` alone (not the `set_color` condition — `NO_COLOR` turns off color, not cursor addressing). Off a TTY they block for the whole budget and print one frame, so a piped consumer never sees a stale frame. Both `--json` paths and `wt prune` still block once and never redraw. `print_stacks_live` threads one `ab_cache` through both frames, since `origin_ahead_behind` is local-only and can't change between them. `wt current` deliberately reports null fields instead of dying outside a worktree — the status bar polls it for arbitrary files.
...
- **Overlapped forge fetches in the mutating commands.** `stack submit` starts its PR/MR query before pushing (a push never changes which PRs exist), and `stack sync` starts its untracked-branch query before the `git fetch`es. Both collect through `collect_prs`, which blocks on `_BLOCKING_PR_TOTAL` and falls back to the all-`None` map `prs_for` would have returned, so a timeout reads as "no PR here" rather than crashing a half-done submit. Two fetches are deliberately **not** hoisted: submit's second `prs_for(order)` must run after the loop creates new PRs, and sync's `fresh_prs` must run after absorption or a retarget aims a base at a just-deleted branch.
