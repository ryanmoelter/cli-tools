...
- Run with `python3 -m unittest discover tests` — stdlib only, no deps, 3.9+.
  ...
  - `test_stack_sync.py` is the exception to the fakes-only rule: it builds a real repo (plus a bare origin) in a tmpdir and drives `cmd_sync` end-to-end, since the rebase arithmetic is the thing under test. It must set `commit.gpgsign=false` — the global config signs via 1Password and would hang.
